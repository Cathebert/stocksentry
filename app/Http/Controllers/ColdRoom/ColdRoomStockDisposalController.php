...
$data['lab_name']='Logged Into: '.$lab->lab_name;
  ...
  return view('cold.inventory.tabs.cold_stockdisposal',$data);
    ...
    //load coldroom stock disposal  report
...
$cost=$term->quantity* $term->cost;
                $nestedData['id']=$x;
                $nestedData['item']=$term->brand;
                $nestedData['brand']= $term->batch_number;
                    $nestedData['batch_number']=$term->code; 
...
//load lab disposals
 public function  loadByLab(Request $request){
   //$lab=auth()->user()->laboratory_id; 
   $lab=$request->lab_id;
   $location="Cold Room";  
    $columns = array(
...
protected function getDisposedRangePrevious($request,$date){
  parse_str($request->expiry_form,$out);
 $expired= $out;

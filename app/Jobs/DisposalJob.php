...
$end   =   date('Y-m-d');
 ...
 $terms = DB::table('inventories as t') 
              ...
              ->select('t.id as id','l.lab_name','s.code','s.brand','s.item_name','t.batch_number','d.dispose_quantity','t.quantity','d.remarks','d.created_at','t.cost','s.item_name','t.expiry_date')
                ->where('d.lab_id',$lab)
          ->whereBetween('d.created_at', [$start, $end])->get();
...
if(count($terms)>0){
    ...
    switch($report->attach_as){
        case 1:
            ...
            $pdf=PDF::loadView('pdf.reports.disposal',['info'=>$terms])->save($path)
        break;
...
$spreadsheet->getActiveSheet()->setCellValue('B7', ' Disposed Item List ');
...
if(!empty($receivers)){
    foreach($receivers as $user){
        ...
        $user->notify(new DisposalSheduleNotification($path,$start, $end));
    }
$mail=new SystemMail();
$mail->lab_id=$lab_id;

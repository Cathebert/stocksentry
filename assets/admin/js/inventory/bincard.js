       ...
       initComplete: function (settings, json) {
         ...
         $("#out").text(json.out);
          ...
          $("#out").text("");
            ...
            console.log(url);
            $("#balance").val(total);
            $("#item_name").html("<i><strong>" + json.item_name + "</strong></i> | Bin Card");
              $("#open").text(total);
...
search:"reload"
        ...
        success: function (data) {
            ...
            if (data.data.length > 0) {
                for (var x = 0; x < data.data.length; x++) {
                    html +=
                        '<div class="input-group-prepend"><span class="input-group-text">'+num+'</span><li class="list-group-item list-group-item-action" style="text-align:left"  id=' +
                        data.data[x]["id"] +
                        ' onclick="CheckBoxSelect(this.id)"><a>' +
                        data.data[x]["item_name"] +
                        "</a></li></div>";
